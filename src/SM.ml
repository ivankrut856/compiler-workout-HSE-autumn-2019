...
let exec (c : config) (order : insn) : config = (* (Printf.printf "%s\n" (show_insn order)); *) 
  match order, c with
    | CONST v, (control_stack, stack, (st, i, o, _)) -> (control_stack, v :: stack, (st, i, o, None))
    | BINOP op, (control_stack, y :: x :: stack, (st, i, o, _)) -> (control_stack, (binop_exec op y x) :: stack, (st, i, o, None))
    | READ, (control_stack, stack, (state, z :: input, output, _)) -> (control_stack, z :: stack, (state, input, output, None))
    | WRITE, (control_stack, z :: stack, (state, input, output, _)) -> (control_stack, stack, (state, input, output @ [z], None))
    | LD s, (control_stack, stack, (state, input, output, _)) -> (control_stack, (Language.State.eval state s) :: stack, (state, input, output, None))
    | ST s, (control_stack, z :: stack, (state, input, output, _)) -> (control_stack, stack, ((Language.State.update s z state), input, output, None))
    | otherwise -> failwith "Attempt to exec control instruction"

let rec eval env (c : config) p = (* Printf.eprintf "%s\n" (if (List.length p) = 0 then "EOP" else (show_insn (head p))); *)
match p with
| [] -> c
...
| (CALL (f, arity, has_value))::ps ->
  let (control_stack, stack, (state, input, output, _)) = c in
  eval env ((ps, state)::control_stack, stack, (state, input, output, None)) (env#labeled f)
| (RET has_value)::ps ->
  let ((ret_prg, ret_state)::control_stack, stack, stm_c) = c in
